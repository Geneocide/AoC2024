...
def generatePermutations(options):
    for permutation in itertools.combinations(options, 2):
        yield permutation
...
def find_antinodes(p1, p2):
    ...
    for ratio in ratios:
        antinode = p1 + ratio * directionVector
...
for frequency, locations in antennas.items():
    for p1, p2 in generatePermutations(locations):
        ...
        for antinode in antinodes:
            ...
            aMap[antinode[1], antinode[0]] = "#"


end_time = time.perf_counter()
prettyPrint(aMap)
print(antennas)
print(f"The number of distance antinodes in range is: {len(antinodes)}")  # 379
